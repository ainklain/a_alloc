...
Data = namedtuple('Data', ['ts_', 'ts_ext_', 'ts_vis_', 'ts', 'ts_ext', 'ts_vis', 'ys', 'ys_', 'ys_vis', 'ys_vis_'])
...
class TimeSeries:
    ...
    def spx_index(self):
        arr = (1+np.array(self.df['spx index'][-500:])).cumprod()
        idx_ = np.sort(np.random.choice(np.arange(50, 400), 50, replace=False))
        ts_ = idx_ / 250
        ts_ext_ = np.array([0.] + list(ts_) + [2.0])
        ts_vis_ = np.linspace(0., 2.0, 500)

        ys_ = arr[idx_].reshape([-1, 1])
        ys_vis_ = arr[:].reshape([-1, 1])
        ...
        ys_vis = torch.tensor(ys_vis_).float().to(device)
        return Data(ts_, ts_ext_, ts_vis_, ts, ts_ext, ts_vis, ys, ys_, ys_vis, ys_vis_)
...
def main():
    # Dataset.
    ts_, ts_ext_, ts_vis_, ts, ts_ext, ts_vis, ys, ys_, ys_vis, ys_vis_ = make_data()
    ...
    if args.show_prior:
        with torch.no_grad():
            ...
            plt.plot(ts_vis_, ys_vis_, color='r')
            plt.ylim(ylims)
            plt.xlabel('$t$')
            plt.ylabel('$Y_t$')
            plt.tight_layout()
