...
def main(cfg: DictConfig):
    ...
    if trainer_cfg.stage1.use:
        ...
        if trainer_cfg.stage1.get('load_checkpoint', False):
            model = model_cls.load_from_checkpoint(checkpoint_path=trainer_cfg.stage1.checkpoint_path, model_cfg=model_cfg, exp_cfg=exp_cfg, dm=dm)
    ...
    if trainer_cfg.stage2.use:
        ...
        trainer.fit(model)
        print("best model: {}".format(trainer.checkpoint_callback.best_model_path))
        trainer.test(model, ckpt_path='best')
