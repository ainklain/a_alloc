...
class MyModel(Module):
    ...
    def __init__(self, n_features, n_assets, configs, ):
        ...
        # self.expected_return_estimator = ExpectedReturnEstimator(c.d_model, n_assets, c.hidden_dim) # TODO: v3 TEST

        # allocator (non-cash only)
        # self.strategies_allocator = StrategiesAllocator(in_dim=n_assets * 3 + c.d_model * 2, out_dim=(n_assets-1+1) * 2, hidden_dim=c.alloc_hidden_dim)
        self.strategies_allocator = StrategiesAllocator(in_dim=n_assets + c.d_model * 2, out_dim=(n_assets-1+1) * 2, hidden_dim=c.alloc_hidden_dim) # TODO: v3 TEST
    ...
    def forward(self, x, wgt, mc_samples):
        """
        wgt = features['wgt']
        wgt = torch.randn([256, 4]).to(tu.device)
        """
        ...
        # _, pred_mu, pred_sigma = self.expected_return_estimator.run(x_emb[:, -1, :], mc_samples=mc_samples) # TODO: v3 TEST
        # print(pred_mu.exp().mean(dim=0), pred_sigma.mean(dim=0))
        # allocation
        # x = torch.cat([pred_mu, pred_sigma, wgt, x_emb[:, -1, :], x_attn], dim=-1) # TODO: v3 TEST
        x = torch.cat([wgt, x_emb[:, -1, :], x_attn], dim=-1)

        wgt_mu, wgt_sigma = self.strategies_allocator(x)

        # return wgt_mu, wgt_sigma, pred_mu, pred_sigma # TODO: v3 TEST
        return wgt_mu, wgt_sigma, None, None

    def noncash_to_all2(self, wgt_, wgt_guide):
        c = self.c
        noncash_idx = np.delete(np.arange(wgt_guide.shape[1]), c.cash_idx)


        wgt_max = torch.tensor(c.wgt_range_max, requires_grad=False).to(wgt_)
        wgt_min = torch.tensor(c.wgt_range_min, requires_grad=False).to(wgt_)
        ...
        wgt_rf = wgt_[:, c.cash_idx:(c.cash_idx+1)]
        wgt_risk = wgt_[:, noncash_idx] / wgt_[:, noncash_idx].sum(dim=-1, keepdim=True) * (1-wgt_rf)

        # 이어붙인다
        wgt_ = torch.cat([wgt_risk[:, :c.cash_idx], wgt_rf, wgt_risk[:, c.cash_idx:]], dim=-1) + 1e-4
    ...
    def noncash_to_all(self, wgt_, wgt_guide):
        ...
        wgt_max = torch.tensor(c.wgt_range_max).to(wgt_)
        wgt_min = torch.tensor(c.wgt_range_min).to(wgt_)
        ...
        if labels is not None:
            ...
            for i_loss, key in enumerate(c.loss_list):
                if key == 'y_pf':
                    ...
                    # losses_dict[key] = nn.MSELoss(reduction='sum')(pred_mu, labels['logy']) # TODO: v3
                    losses_dict[key] = 0
                elif key == 'wgt_guide':
