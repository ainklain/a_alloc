...
class MyModel(Module):
    ...
    def forward(self, features, wgt, mc_samples):
        ...
        x = torch.cat([features['idx'], features['macro']], dim=-1)
        n_batch = x.shape[0]
        pred, pred_mu, pred_sigma = self.expected_return_estimator.run(x, mc_samples=mc_samples)
    ...
    def forward_with_loss(self, features, labels=None,
        ...
        if labels is not None:
            ...
            for i_loss, key in enumerate(c.loss_list):
                if key == 'y_pf':
                    losses_dict[key] = -((x - features['wgt']) * next_y).sum()
                elif key == 'mdd_pf':
                    ...
                    if labels_prev is not None:
                        ...
                        wgt_prev = prev_x * (1 + next_y_prev)
                        wgt_prev = wgt_prev / wgt_prev.sum(dim=1, keepdim=True)
                        losses_dict[key] = (torch.abs(x - wgt_prev) * c.cost_rate).sum()
                    else:
                        losses_dict[key] = (torch.abs(x - features['wgt']) * c.cost_rate).sum()
                elif key == 'entropy':
                    if c.max_entropy:
